...
const cohere = new CohereClient({
  token: "7Dbjk8G3UCx4QSkXxuApvNpPH4AFKaW1aOy743z3", // Replace with your actual API key
});

console.log("Cohere initialized:", cohere);
...
async function analyzeText(text, task) {
  ...
  return (
    response.data.generations[0]?.text.trim() || "No relevant insights found."
  );
}
...
router.post("/analyze-pdf", upload.single("pdf"), async (req, res) => {
  ...
  try {
    ...
    const [
      ...
      analyzeText(
        truncatedText,
        "Summarize the key points and provide a brief overview."
      ),
      analyzeText(
        truncatedText,
        "Identify and prioritize the most critical tasks with justifications."
      ),
      analyzeText(
        truncatedText,
        "Suggest an optimal approach for resource allocation and justify the distribution."
      ),
      analyzeText(
        truncatedText,
        "Analyze potential risks and suggest mitigation strategies."
      ),
      analyzeText(
        truncatedText,
        "Evaluate possible errors and provide a plan to minimize rework."
      ),
      analyzeText(
        truncatedText,
        "Propose strategies to optimize business costs and improve efficiency."
      ),
    ]);
    ...
    res
      .status(500)
      .json({ error: "Error analyzing PDF", details: error.message });
  }
});
...
// Function to split text into smaller chunks (Cohere allows larger inputs)
...
router.post("/summarize", upload.single("pdf"), async (req, res) => {
  ...
  try {
    const pdfText = await pdfParse(req.file.buffer);
    const fullPdfText = pdfText.text;

    const textChunks = splitText(fullPdfText, 2000);
    let summaries = [];
    ...
    for (const chunk of textChunks) {
      ...
      summaries.push(response.summary); // Fix: response.summary instead of response.body.summary
    }

    res.json({ summary: summaries.join(" "), fullText: fullPdfText });
  } catch (error) {
    console.error("Summarization Error:", error);
    res.status(500).json({ error: "Error summarizing PDF" });
  }
...
router.post("/ask", async (req, res) => {
  ...
  try {
    ...
    const finalAnswer =
      answers.find((ans) => ans) || "No relevant answer found.";
